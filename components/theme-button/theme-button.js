class ThemeButtonElement extends HTMLElement {
	constructor() {
		super();
    ...
    `;
		...
		if (storedTheme) {
			...
			BUTTON.setAttribute('aria-pressed', storedTheme === 'dark-mode'); // Update button state
		}
		...
		BUTTON.addEventListener('click', () => {
			...
			localStorage.setItem('theme', IS_PRESSED ? '' : 'dark-mode');
		});
		...
		// 	// Store the updated theme preference
		// 	localStorage.setItem('theme', IS_PRESSED ? 'dark-mode' : ''); // Store 'dark-mode' when pressed
		// });
		...
		// 			// Store the updated theme preference
		// 			localStorage.setItem('theme', IS_PRESSED ? 'dark-mode' : ''); // Remove theme class if toggled off
		// 		});
	}
}

customElements.define('theme-button', ThemeButtonElement);
